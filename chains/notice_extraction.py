...
info_parse_prompt = ChatPromptTemplate.from_messages(
    [
        (
            ...
            maximum potential fine from the message.

            - If any fields are missing, leave them blank.
            - Format all dates as YYYY-MM-DD.
            - Ensure `project_id` is returned as an **integer**, not a string.
            - Ensure `max_potential_fine` is a number.
            - Only include fields that are clearly found in the message.
...
structured_model = model.with_structured_output(NoticeEmailExtract)

NOTICE_PARSER_CHAIN = info_parse_prompt | structured_model